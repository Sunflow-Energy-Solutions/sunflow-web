...
export default function ResidentialChargerTable() {
  const { addItem } = useCart();
  const prices = residentialChargers.map((c) => c.price).filter((p): p is number => p !== null);
  const priceMin = Math.min(...prices);
  const priceMax = Math.max(...prices);
  const ipRatings = Array.from(new Set(residentialChargers.map((c) => c.ipRating))).sort();

  const [phase, setPhase] = useState<string | null>(null);
  const [ipRating, setIpRating] = useState<string | null>(null);
  const [solarSmartOnly, setSolarSmartOnly] = useState(false);
  const [loadManagementOnly, setLoadManagementOnly] = useState(false);
  const [phaseSwitchingOnly, setPhaseSwitchingOnly] = useState(false);
  ...
  const filtered = useMemo(() => {
    ...
    const list = residentialChargers.filter((c) => {
      if (phase && c.phase !== phase) return false;
      if (ipRating && c.ipRating !== ipRating) return false;
      if (solarSmartOnly && !c.solarSmart) return false;
      if (loadManagementOnly && !c.loadManagement) return false;
      if (phaseSwitchingOnly && !c.phaseSwitching) return false;
  ...
  }, [phase, ipRating, solarSmartOnly, loadManagementOnly, phaseSwitchingOnly, ocppOnly, bidirectionalOnly, maxPrice, sort, priceMax]);

  const resetFilters = () => {
    setPhase(null);
    setIpRating(null);
    setSolarSmartOnly(false);
    setLoadManagementOnly(false);
    setPhaseSwitchingOnly(false);
    setOcppOnly(false);
    ...
    <div className="space-y-6 rounded-3xl border border-mist-200 bg-white p-6">
      ...
      <div>
        ...
        <div className="mt-2.5 space-y-2">
          <FilterCheckbox label="Solar Smart Charging" checked={solarSmartOnly} onChange={setSolarSmartOnly} />
          <FilterCheckbox label="Load Balancing" checked={loadManagementOnly} onChange={setLoadManagementOnly} />
          <FilterCheckbox label="3-to-1 Phase Switching" checked={phaseSwitchingOnly} onChange={setPhaseSwitchingOnly} />
          <FilterCheckbox label="OCPP 1.6 Compatible" checked={ocppOnly} onChange={setOcppOnly} />
              ...
              <table className="w-full min-w-[2500px] border-collapse text-left text-sm">
                <thead>
                  <tr className="bg-navy-950 text-white">
                    ...
                    <th scope="col" className={HEADER_CELL}>Solar Smart</th>
                    <th scope="col" className={HEADER_CELL}>Load Bal.</th>
                    <th scope="col" className={HEADER_CELL}>3&harr;1 Phase</th>
                    ...
                    <th scope="col" className={HEADER_CELL}>Charging Modes</th>
                ...
                <tbody className="divide-y divide-mist-200">
                  {filtered.map((c, i) => (
                    <tr key={c.id} className={i % 2 === 0 ? "bg-white" : "bg-mist-50"}>
                      ...
                      <td className="px-4 py-3.5"><FeatureIcon active={c.solarSmart ?? false} label="Solar Smart Charging" /></td>
                      <td className="px-4 py-3.5"><FeatureIcon active={c.loadManagement} label="Load Balancing" /></td>
                      <td className="px-4 py-3.5"><FeatureIcon active={c.phaseSwitching} label="3-to-1 Phase Switching" /></td>
                      ...
                      <td className="w-56 whitespace-normal px-4 py-3.5 text-mist-600">{c.chargingModes}</td>
